...
/**
 * Test e2e de la case « Exclure de la fidélité » sur la PRODUCTION (script à
 * jeton, à usage unique).
 *
 * Déposé à la racine du thème sous `_e2e-exclusion.php` par
 * `scripts/e2e-exclusion-prod.sh`, aux côtés du cœur partagé
 * `_e2e-exclusion-core.php` (car `tools/` n'existe pas en prod). Appelé en HTTPS
 * avec le jeton, puis les deux fichiers sont supprimés.
 *
 * Sûr pour la prod : voir l'en-tête de `tools/e2e-exclusion.php` (produit +
 * commande isolés, aucun e-mail, aucune recette, tout nettoyé).
 *
...
require __DIR__ . '/_e2e-exclusion-core.php';

$run = luziapi_e2e_exclusion_run();
