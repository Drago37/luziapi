#!/usr/bin/env bash
#
# Lance le test e2e de la validation de zone de livraison sur la PRODUCTION.
#
#   scripts/e2e-delivery-zone-prod.sh
#
# Dépose le cœur partagé (`_e2e-delivery-zone-core.php`) et le wrapper à jeton
# (`_e2e-delivery-zone.php`) à la racine du thème, appelle le wrapper en HTTPS,
# affiche le résumé, puis supprime les deux. Aucune commande ni e-mail : seul le
# hook de validation est exécuté avec un WP_Error en mémoire.
...
CORE="${THEME}/tools/e2e-delivery-zone.php"
WRAPPER="${THEME}/tools/e2e-delivery-zone-prod.php"
URL="https://www.luziapi.fr/wp-content/themes/luziapi/_e2e-delivery-zone.php"
...
echo "ℹ️   Test en lecture seule : aucune commande, aucun e-mail (hook de validation seul)."

TOKEN="$(openssl rand -hex 16)"

WORK="$(mktemp -d)"
trap 'rm -rf "${WORK}"' EXIT
LOCAL_CORE="${WORK}/_e2e-delivery-zone-core.php"
LOCAL_WRAPPER="${WORK}/_e2e-delivery-zone.php"
cp "${CORE}" "${LOCAL_CORE}"
...
ftp_do "rm _e2e-delivery-zone.php; rm _e2e-delivery-zone-core.php;" || echo "⚠️   Suppression à vérifier manuellement."
...
PY
then
  echo "✔  Test e2e zone de livraison prod OK."
else
  echo "✖  Test e2e zone de livraison prod en échec ou réponse inattendue :"
  printf '%s\n' "${RESULT}" | head -c 800
  exit 1
fi
