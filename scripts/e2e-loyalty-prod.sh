#!/usr/bin/env bash
#
# Lance le test e2e de la FIDÉLITÉ sur la PRODUCTION.
#
#   scripts/e2e-loyalty-prod.sh
#
# Dépose un script à jeton à usage unique à la racine du thème, l'appelle en
# HTTPS (jeton embarqué), affiche le résumé, puis supprime le script dans tous
# les cas. Le test crée UN produit masqué + UNE commande isolés, et les supprime
# en fin de run : aucun e-mail, aucune recette, aucune donnée réelle touchée.
...
SCRIPT="${THEME}/tools/e2e-loyalty-prod.php"
URL="https://www.luziapi.fr/wp-content/themes/luziapi/_e2e-loyalty.php"
...
ftp_do "rm _e2e-loyalty.php;" || echo "⚠️   Suppression à vérifier manuellement."
...
PY
then
  echo "✔  Test e2e fidélité prod OK."
else
  echo "✖  Test e2e fidélité prod en échec ou réponse inattendue :"
  printf '%s\n' "$RESULT" | head -c 800
  exit 1
fi
