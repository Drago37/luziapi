#!/usr/bin/env bash
#
# Lance le test e2e « recette retirée à la corbeille/suppression de la commande »
# sur la PRODUCTION.
#
#   scripts/e2e-orphan-receipt-prod.sh
#
# Dépose DEUX fichiers à usage unique à la racine du thème — le cœur partagé
# (`_e2e-orphan-receipt-core.php`) et le wrapper à jeton (`_e2e-orphan-receipt.php`) —
# appelle le wrapper en HTTPS (jeton embarqué), affiche le résumé, puis supprime les
# deux dans tous les cas. Produit masqué + commandes isolés, aucun e-mail, tout nettoyé.
...
CORE="${THEME}/tools/e2e-orphan-receipt.php"
WRAPPER="${THEME}/tools/e2e-orphan-receipt-prod.php"
URL="https://www.luziapi.fr/wp-content/themes/luziapi/_e2e-orphan-receipt.php"
...
echo "ℹ️   Test isolé : produit masqué + commandes de test, aucun e-mail, tout est nettoyé."

TOKEN="$(openssl rand -hex 16)"

WORK="$(mktemp -d)"
trap 'rm -rf "${WORK}"' EXIT
LOCAL_CORE="${WORK}/_e2e-orphan-receipt-core.php"
LOCAL_WRAPPER="${WORK}/_e2e-orphan-receipt.php"
cp "${CORE}" "${LOCAL_CORE}"
...
ftp_do "rm _e2e-orphan-receipt.php; rm _e2e-orphan-receipt-core.php;" || echo "⚠️   Suppression à vérifier manuellement."
...
PY
then
  echo "✔  Test e2e recette orpheline prod OK."
else
  echo "✖  Test e2e recette orpheline prod en échec ou réponse inattendue :"
  printf '%s\n' "${RESULT}" | head -c 800
  exit 1
fi
