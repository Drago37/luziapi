#!/usr/bin/env bash
#
# Lance le test e2e du process de commande sur la PRODUCTION.
#
#   scripts/e2e-prod.sh          → dry-run (aucun e-mail expédié)
#   scripts/e2e-prod.sh --send   → e-mails RÉELS vers l'adresse d'identité
#
...
# dans tous les cas. Lit DEPLOY_FTP_* dans .env.local et l'identité dans
# www/wp-content/themes/luziapi/tools/.e2e-identity.json.
#
# Aucun secret ni donnée personnelle n'est écrit dans ce fichier.
...
SCRIPT="$THEME/tools/e2e-orders.php"
IDENTITY="$THEME/tools/.e2e-identity.json"
URL="https://www.luziapi.fr/wp-content/themes/luziapi/_e2e-orders.php"
...
if $SEND; then
    echo "⚠️   Mode E-MAILS RÉELS : des e-mails de test vont partir vers l'adresse d'identité."
else
    echo "ℹ️   Mode dry-run : aucun e-mail ne sera expédié."
fi
...
ftp_do "rm _e2e-orders.php;" || echo "⚠️   Suppression à vérifier manuellement."
...
print(d["mode"], "|", d["summary"], "| all_passed:", d["all_passed"])
if d.get("fatal_error"):
    print("FATAL:", d["fatal_error"])
for r in d["results"]:
    if not r["ok"]:
        print("  ✗", r["label"], r.get("detail", ""))
print("cleanup:", d["cleanup"])
sys.exit(0 if d["all_passed"] and not d.get("fatal_error") else 1)
PY
then
    echo "✔  Test e2e prod OK."
else
    echo "✖  Test e2e prod en échec ou réponse inattendue :"
    printf '%s\n' "$RESULT" | head -c 800
    exit 1
fi
