#!/usr/bin/env bash
#
# Lance le test e2e de la case « Exclure de la fidélité » sur la PRODUCTION.
#
#   scripts/e2e-exclusion-prod.sh
#
# Dépose DEUX fichiers à usage unique à la racine du thème — le cœur partagé
# (`_e2e-exclusion-core.php`, car `tools/` n'existe pas en prod) et le wrapper à
# jeton (`_e2e-exclusion.php`) — appelle le wrapper en HTTPS (jeton embarqué),
...
# recette, aucune donnée réelle touchée.
...
CORE="${THEME}/tools/e2e-exclusion.php"
WRAPPER="${THEME}/tools/e2e-exclusion-prod.php"
URL="https://www.luziapi.fr/wp-content/themes/luziapi/_e2e-exclusion.php"
...
echo "ℹ️   Test isolé : produit masqué + commande de test, aucun e-mail, tout est nettoyé."

TOKEN="$(openssl rand -hex 16)"

WORK="$(mktemp -d)"
trap 'rm -rf "${WORK}"' EXIT
LOCAL_CORE="${WORK}/_e2e-exclusion-core.php"
LOCAL_WRAPPER="${WORK}/_e2e-exclusion.php"
cp "${CORE}" "${LOCAL_CORE}"
...
ftp_do "rm _e2e-exclusion.php; rm _e2e-exclusion-core.php;" || echo "⚠️   Suppression à vérifier manuellement."
...
PY
then
  echo "✔  Test e2e exclusion fidélité prod OK."
else
  echo "✖  Test e2e exclusion fidélité prod en échec ou réponse inattendue :"
  printf '%s\n' "${RESULT}" | head -c 800
  exit 1
fi
