#!/usr/bin/env bash
#
# Audit de dérive recette ↔ commandes sur la PRODUCTION (lecture seule).
#
#   scripts/audit-receipts-prod.sh
#   LUZIAPI_AUDIT_YEAR=2026 scripts/audit-receipts-prod.sh
#
# Dépose deux fichiers à usage unique à la racine du thème — le cœur partagé
# (`_audit-receipt-drift-core.php`) et le wrapper à jeton (`_audit-receipt-drift.php`) —
...
CORE="${THEME}/tools/audit-receipt-drift-core.php"
WRAPPER="${THEME}/tools/audit-receipt-drift-prod.php"
URL="https://www.luziapi.fr/wp-content/themes/luziapi/_audit-receipt-drift.php"
...
LOCAL_CORE="${WORK}/_audit-receipt-drift-core.php"
LOCAL_WRAPPER="${WORK}/_audit-receipt-drift.php"
cp "${CORE}" "${LOCAL_CORE}"
...
ftp_do "rm _audit-receipt-drift.php; rm _audit-receipt-drift-core.php;" || echo "⚠️   Suppression à vérifier manuellement."
...
print(f"Audit de dérive recette ↔ commandes — {scope}")
if not d.get("has_drift"):
    print("✔  Aucune dérive : recettes et commandes concordent.")
    sys.exit(0)
for r in d.get("missing", []):
    print(f"  ⚠  Commande {r['order']} (#{r['id']}) sans recette : attendu {euros(r['expected'])}, enregistré {euros(r['recorded'])}")
for r in d.get("divergent", []):
    print(f"  ⚠  Commande {r['order']} (#{r['id']}) montant divergent : écart {euros(r['difference'])}")
for r in d.get("orphans", []):
    print(f"  ⚠  Recette orpheline : commande #{r['order_id']} disparue, {euros(r['net'])} encore au registre")
print(f"✖  {d['anomaly_count']} anomalie(s), dérive totale {euros(d['total_drift_cents'])}.")
sys.exit(1)
PY
then
