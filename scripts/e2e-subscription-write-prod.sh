#!/usr/bin/env bash
#
# Lance le test e2e « écriture d'abonnement Brevo » sur la PRODUCTION.
#
#   scripts/e2e-subscription-write-prod.sh
#
# Dépose DEUX fichiers à usage unique à la racine du thème — le cœur partagé
# (`_e2e-subscription-write-core.php`) et le wrapper à jeton (`_e2e-subscription-write.php`) —
# appelle le wrapper en HTTPS (jeton embarqué), affiche le résumé, puis supprime les deux dans
# tous les cas. Appels Brevo interceptés (réponse factice), aucune requête réelle, aucun
# contact touché.
...
CORE="${THEME}/tools/e2e-subscription-write.php"
WRAPPER="${THEME}/tools/e2e-subscription-write-prod.php"
URL="https://www.luziapi.fr/wp-content/themes/luziapi/_e2e-subscription-write.php"
...
echo "ℹ️   Test isolé : appels Brevo interceptés (réponse factice), aucune requête réelle, aucun contact touché."

TOKEN="$(openssl rand -hex 16)"

WORK="$(mktemp -d)"
trap 'rm -rf "${WORK}"' EXIT
LOCAL_CORE="${WORK}/_e2e-subscription-write-core.php"
LOCAL_WRAPPER="${WORK}/_e2e-subscription-write.php"
cp "${CORE}" "${LOCAL_CORE}"
...
ftp_do "rm _e2e-subscription-write.php; rm _e2e-subscription-write-core.php;" || echo "⚠️   Suppression à vérifier manuellement."
...
PY
then
  echo "✔  Test e2e écriture d’abonnement Brevo prod OK."
else
  echo "✖  Test e2e écriture d’abonnement Brevo prod en échec ou réponse inattendue :"
  printf '%s\n' "${RESULT}" | head -c 800
  exit 1
fi
