#!/usr/bin/env bash
#
# Lance le test e2e « répertoire d'abonnés (Brevo, lecture seule) » sur la PRODUCTION.
#
#   scripts/e2e-subscribers-prod.sh
#
# Dépose DEUX fichiers à usage unique à la racine du thème — le cœur partagé
# (`_e2e-subscribers-core.php`) et le wrapper à jeton (`_e2e-subscribers.php`) — appelle
...
CORE="${THEME}/tools/e2e-subscribers.php"
WRAPPER="${THEME}/tools/e2e-subscribers-prod.php"
URL="https://www.luziapi.fr/wp-content/themes/luziapi/_e2e-subscribers.php"
...
LOCAL_CORE="${WORK}/_e2e-subscribers-core.php"
LOCAL_WRAPPER="${WORK}/_e2e-subscribers.php"
cp "${CORE}" "${LOCAL_CORE}"
...
ftp_do "rm _e2e-subscribers.php; rm _e2e-subscribers-core.php;" || echo "⚠️   Suppression à vérifier manuellement."
...
PY
then
  echo "✔  Test e2e abonnés prod OK."
else
  echo "✖  Test e2e abonnés prod en échec ou réponse inattendue :"
  printf '%s\n' "${RESULT}" | head -c 800
  exit 1
fi
