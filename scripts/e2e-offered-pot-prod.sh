#!/usr/bin/env bash
#
# Lance le test e2e « Ajouter un pot offert à une commande existante » sur la PRODUCTION.
#
#   scripts/e2e-offered-pot-prod.sh
#
# Dépose DEUX fichiers à usage unique à la racine du thème — le cœur partagé
# (`_e2e-offered-pot-core.php`, car `tools/` n'existe pas en prod) et le wrapper à
# jeton (`_e2e-offered-pot.php`) — appelle le wrapper en HTTPS (jeton embarqué),
# affiche le résumé, puis supprime les deux dans tous les cas. Le test crée UN
# produit masqué + UNE commande isolés et les supprime : aucun e-mail, aucune
# recette, aucun montant modifié, aucune donnée réelle touchée.
...
CORE="${THEME}/tools/e2e-offered-pot.php"
WRAPPER="${THEME}/tools/e2e-offered-pot-prod.php"
URL="https://www.luziapi.fr/wp-content/themes/luziapi/_e2e-offered-pot.php"
...
echo "ℹ️   Test isolé : produit masqué + commande de test, aucun e-mail, aucun montant modifié, tout est nettoyé."

TOKEN="$(openssl rand -hex 16)"

WORK="$(mktemp -d)"
trap 'rm -rf "${WORK}"' EXIT
LOCAL_CORE="${WORK}/_e2e-offered-pot-core.php"
LOCAL_WRAPPER="${WORK}/_e2e-offered-pot.php"
cp "${CORE}" "${LOCAL_CORE}"
...
ftp_do "rm _e2e-offered-pot.php; rm _e2e-offered-pot-core.php;" || echo "⚠️   Suppression à vérifier manuellement."
...
PY
then
  echo "✔  Test e2e pot offert prod OK."
else
  echo "✖  Test e2e pot offert prod en échec ou réponse inattendue :"
  printf '%s\n' "${RESULT}" | head -c 800
  exit 1
fi
